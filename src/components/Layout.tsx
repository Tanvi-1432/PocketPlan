  ...
  return (
    <button
      ...
      className={`flex items-center gap-2 rounded-xl transition-all duration-200 active:scale-95
        text-slate-400 hover:text-violet-600 hover:bg-violet-50/60
        dark:text-slate-500 dark:hover:text-violet-400 dark:hover:bg-white/5
...
export default function Layout({ currentPage, onNavigate, children }: LayoutProps) {
  ...
  const navButtonClass = (id: Page) => {
    ...
    return `w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-200 ${
      active
        ? 'nav-active'
        : 'text-slate-500 hover:bg-violet-50/50 hover:text-slate-700 dark:text-slate-400 dark:hover:bg-white/5 dark:hover:text-slate-200 active:scale-[0.98]'
    }`
  }

  return (
    <div className="app-bg min-h-dvh flex transition-colors duration-300 relative">

      {/* ── Floating sidebar ─────────────────────────────────────────────── */}
      {/* Sits inside a 16px margin on all sides, full height minus margin */}
      <aside className="hidden md:flex flex-col w-[212px] glass-sidebar fixed z-20 transition-all duration-300"
        style={{ top: 16, left: 16, bottom: 16, height: 'calc(100dvh - 32px)' }}>
        ...
        <div className="px-5 pt-6 pb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <span className="text-[15px] font-semibold tracking-tight leading-none text-slate-800 dark:text-slate-100">
                PocketPlan
              </span>
        ...
        {/* Divider */}
        <div className="mx-4 h-px bg-gradient-to-r from-transparent via-violet-200/60 to-transparent dark:via-white/8 mb-2" />
      ...
      {/* ── Mobile top bar ───────────────────────────────────────────────── */}
      <div className="md:hidden fixed top-0 inset-x-0 z-30 mobile-topbar flex items-center justify-between px-4 h-14">
        ...
        <div className="flex items-center gap-1">
          ...
          <button
            ...
            className="p-2 rounded-xl text-slate-500 dark:text-slate-400 hover:bg-violet-50/60 dark:hover:bg-white/8 active:scale-95 transition-all duration-150"
      ...
      <div
        className={`md:hidden fixed top-14 left-3 right-3 z-30 glass-card px-3 py-2 space-y-0.5 transition-all duration-250 ease-out ${
      ...
      >
        ...
        <div className="pt-2 pb-1 space-y-1 mt-1"
      ...
      {/* ── Main content ─────────────────────────────────────────────────── */}
      {/* Offset by sidebar width (212) + margin (16) + gap (16) = 244px */}
      <main className="flex-1 md:ml-[244px] pt-14 md:pt-0 min-h-dvh relative z-10">
        <div
      ...
      <div className="md:hidden fixed bottom-0 inset-x-0 z-30 flex justify-center"
        ...
        <nav className="mobile-nav-glass flex items-center gap-0 px-2 py-1.5 w-full max-w-sm">
          {MOBILE_NAV_ITEMS.map((item) => {
            ...
            return (
              <button
                ...
                className={`flex-1 flex flex-col items-center gap-1 py-1.5 rounded-full transition-all duration-200 active:scale-90 ${
                  active
              ...
              >
                {active && (
                  <div className="absolute w-8 h-8 rounded-full bg-violet-100/80 dark:bg-violet-500/15 -z-10 blur-sm" />
                )}
          ...
          <button
            onClick={() => setMobileMenuOpen((o) => !o)}
            className={`flex-1 flex flex-col items-center gap-1 py-1.5 rounded-full transition-all duration-200 active:scale-90 ${
              mobileMenuOpen ? 'text-violet-600 dark:text-violet-400' : 'text-slate-400 dark:text-slate-500'
            }`}
          >
