    ...
    <div className="flex flex-col gap-6">
      ...
      {accounts.length === 0 && !isSyncing && (
        <div className="flex flex-col items-center justify-center py-24 text-center">
          <div className="w-16 h-16 bg-slate-100 dark:bg-slate-800 rounded-2xl flex items-center justify-center mb-4">
            <svg className="w-8 h-8 text-slate-400 dark:text-slate-500" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.5}>
      ...
      {checkingAccounts.length > 0 && (
        <section className="flex flex-col gap-3">
          <h2 className="text-xs font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wider">Bank Accounts</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {checkingAccounts.map((a) => <AccountCard key={a.id} account={a} transactionCount={txCountByAccount.get(a.id) ?? 0} />)}
          </div>
        </section>
      )}
      ...
      {creditAccounts.length > 0 && (
        <section className="flex flex-col gap-3">
          <h2 className="text-xs font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wider">Credit Cards</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {creditAccounts.map((a) => <AccountCard key={a.id} account={a} transactionCount={txCountByAccount.get(a.id) ?? 0} />)}
          </div>
        </section>
      )}
      ...
      {investmentAccounts.length > 0 && (
        <section className="flex flex-col gap-3">
          <h2 className="text-xs font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wider">Investment Accounts</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {investmentAccounts.map((a) => <AccountCard key={a.id} account={a} transactionCount={txCountByAccount.get(a.id) ?? 0} />)}
          </div>
        </section>
      )}
    </div>
