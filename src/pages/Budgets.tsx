    ...
    <div className="flex flex-col gap-6">
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-0.5">Set monthly spending limits per category</p>
        </div>
        <Button onClick={openAdd}>+ Set budget</Button>
      </div>

      <div className="flex flex-col sm:flex-row sm:items-center gap-3">
        <Select options={monthOptions} value={selectedMonth} onChange={(e) => setSelectedMonth(e.target.value)} className="sm:w-48" aria-label="Select month" />
        {overBudgetCount > 0 && (
      ...
      {progressItems.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-24 text-center">
          ...
          <p className="text-base font-semibold text-slate-700 dark:text-slate-300">No budgets for {formatMonth(selectedMonth)}</p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {progressItems.map((item) => (
            <BudgetCard key={item.budget.id} item={item} onEdit={() => openEdit(item.budget)} onDelete={() => deleteBudget(item.budget.id)} />
          ))}
        </div>
      )}

      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title={editing ? 'Edit budget' : 'Set budget'}>
        <BudgetForm initial={editing ?? undefined} onSubmit={handleSubmit} onCancel={() => setModalOpen(false)} />
      </Modal>
    </div>
  )
}
